...
def find_games(request):
    ...
    for tick in by_tick:
        ...
        # These values are numpy 64 int and numpy float, which cause errors in JsonResponse
        # Have to cast to native python type so they can be included in the response
        win_reason = tick.iloc[0]['win_reason']
        t_score = int(tick.iloc[0]['t_score'])
        ct_score = int(tick.iloc[0]['ct_score'])
        hltv_link = match_links.loc[(match_links.id == tick.iloc[0]['match_id'])]['hltv_link'].values[0]
        time = float(tick.iloc[0]['time'])
